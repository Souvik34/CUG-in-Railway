...
exports.deactivate = async (req, res) => {
  ...
  try {
    const cug = await Add_cug.findOne({ cugNo });
    if (cug) {
      console.log(`Found CUG No. ${cugNo}:`, cug);
      if (cug.status === 'inactive') {
        return res.status(400).json({ message: 'CUG No. is already inactive' });
      }

      const deactivatedCug = new Deactivated_cug(cug.toObject());
      await deactivatedCug.save();
      console.log(`Saved to Deactivated_cug collection:`, deactivatedCug);

      await Add_cug.deleteOne({ cugNo });
      console.log(`Deleted from Add_cug collection: CUG No. ${cugNo}`);
